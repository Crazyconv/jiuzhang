...
public class BookingSystem {
	...
	public List<Hotel> searchHotel(SearchHotelRequest request)
	{
        ...
        for (Hotel hotel : this.hotels)
        {
            ...
            int capacity = 0;
            for (Entry<RoomType, List<Room>> entry : searchResult.entrySet())
            {
                capacity += entry.getKey().getCapacity() * entry.getValue().size();
            }
            if (capacity >= request.getGroupSize())
                result.add(hotel);
        }
        return result;
	}
	...
	{
        Reservation res = hotel.makeReservation(request);
        res.setHotel(hotel);
        return res;
	}
